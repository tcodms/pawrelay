...
4. 보호소 담당자에게 매칭 결과 확인 알림 → 24시간 내 승인/거절 가능
   - 12시간 경과 시 리마인더 알림 재발송 (BE 처리, FE는 WebSocket 이벤트 수신 후 뱃지 업데이트)
   - 미응답 시 자동 승인(`auto_approved`) 처리 후 1시간 이내 취소 가능 (grace period)
   - FE 대시보드 상태별 UI: 대기 중 → 승인/거절 버튼 + 카운트다운 / auto_approved → "자동 승인됨" + 1시간 취소 버튼 / 기간 종료 → 버튼 숨김
5. 봉사자에게 매칭 제안 → 24시간 내 수락/거절 (기산점: 알림 발송 시각, API에 `notified_at` 필드 필요)
   - **FE 알림 진입점: 챗봇이 단일 허브**
     - Web Push / 인앱 토스트 수신 → 해당 챗봇 세션으로 딥링크
     - 챗봇 내에서 매칭 제안 버블(matching_reason + 수락/거절) 표시
     - "상세 보기" → `/volunteer/matching/{segment_id}` (인계 장소 전체 목록, 카카오맵 등)
6. 확정 시 volunteer_schedules status → matched,
   아래 내용 포함한 매칭 확정 알림 발송
   - **알림 채널:** Web Push(백그라운드) + 인앱 토스트(포그라운드) → 챗봇 세션으로 딥링크
   - 챗봇 내 매칭 확정 버블에서 내용 확인:
     - 담당 구간 (예: 광주 → 천안)
     - 인계 장소 추천 목록 (type별 그룹: 기차역 / 휴게소 / 버스터미널)
     - 카카오 오픈채팅 링크 (구간별 채팅방) — 장소 협의용
     - 카카오맵 링크
     - 파트너 연락처·6자리 인계 코드 (출발 당일 00:00 이전 잠금)
...
- 보호소 서류 업로드 (위탁계약서, 보호소 등록증 등) + 관리자 수동 인증 (국가동물보호정보시스템 교차 확인)
...
- 가입 시 보호소 서류 업로드 (위탁계약서, 보호소 등록증 등) → S3 저장
- 관리자가 국가동물보호정보시스템에서 보호소명 교차 확인 후 수동 승인
- 관리자 수동 승인 (`/admin/shelters/{id}/approve`) + 검토 메모 기록
