...
챗봇 대화 흐름은 LLM 없이 사전 정의된 상태 머신으로 처리한다. 수집 항목이 구조화된 필드이므로 LLM 호출 없이 단계별 질문으로 처리하며, 파싱 안정성과 응답 속도를 확보한다.

```
State: ASK_ROUTE → ASK_DATE → ASK_VEHICLE → ASK_ANIMAL_SIZE → CONFIRM
```

게시판에서 post_id와 함께 진입한 경우, 공고에 이미 포함된 항목은 자동 입력되고 해당 질문 state를 건너뛴다.

| 수집 항목                 | 게시판 진입 시                       | 직접 진입 시 |
| ------------------------- | ------------------------------------ | ------------ |
| 이동 날짜                 | 공고에서 자동 입력                   | 직접 입력    |
| 탑승 가능 동물 크기       | 공고에서 자동 입력                   | 직접 입력    |
| 담당 구간 (출발지→목적지) | 직접 입력 (공고 전체 경로 참고 표시) | 직접 입력    |
| 차량 유무                 | 직접 입력                            | 직접 입력    |

> 게시판 진입 시 챗봇 상단에 공고 요약 카드(날짜, 동물 크기, 출발지→목적지)가 자동으로 표시된다.
...
| 역할         | 담당 영역                                                         |
| ------------ | ----------------------------------------------------------------- |
| **Backend**  | FastAPI, DB/PostGIS, Redis, 배치 스케줄러, 알림 인프라, API       |
| **Frontend** | Next.js/PWA, 화면 구현, 카카오맵, Web Push 수신 처리              |
| **AI**       | 챗봇 상태 머신, LLM 매칭 엔진, 프롬프트, 체인 알고리즘, 이상 감지 |

---

### 1주차: 프로젝트 세팅 + 인터페이스 합의

> **목표:** 세 팀이 독립적으로 작업 가능한 기반 구축. 이번 주 합의한 인터페이스는 이후 변경 금지.

| 역할         | 작업                                                                                           |
| ------------ | ---------------------------------------------------------------------------------------------- |
| **공통**     | Git 브랜치 전략 확정, 개발/스테이징 환경 세팅                                                  |
| **Backend**  | DB 스키마 전체 구현 (PostgreSQL + PostGIS), FastAPI 프로젝트 구조 세팅                         |
| **Frontend** | Next.js 프로젝트 세팅, 라우팅 구조 (`/dashboard`, `/volunteer`, `/public`) 확정, 공통 컴포넌트 |
| **AI**       | 챗봇 상태 머신 설계 문서 작성, LLM 추상화 레이어 (`ClaudeProvider / OpenAIProvider`) 구현      |

**주차 말 합의 항목 (필수)**

- API Request/Response 스키마 (TypeScript 타입 or OpenAPI 스펙)
- WebSocket 이벤트 포맷
- 챗봇 상태 머신 인터페이스 (AI → Backend 전달 구조)

---

### 2주차: 인증

> **목표:** 로그인/가입 완성. 이후 모든 기능이 인증 위에서 동작.

| 역할         | 작업                                                                                                 |
| ------------ | ---------------------------------------------------------------------------------------------------- |
| **Backend**  | JWT Auth API 전체 구현 (회원가입, 로그인, 이메일 인증, Refresh Token, 보호소 사업자번호 검증 플로우) |
| **Frontend** | 봉사자/보호소 회원가입 페이지, 로그인 페이지, JWT 토큰 관리 (httpOnly 쿠키 or 메모리), 인증 미들웨어 |
| **AI**       | 챗봇 상태 머신 구현 시작 (ASK_ROUTE → ASK_DATE → ASK_VEHICLE → ASK_ANIMAL_SIZE → CONFIRM)            |

---

### 3주차: 공고 + 챗봇

> **목표:** 보호소가 공고를 올리고, 봉사자가 챗봇으로 동선을 등록할 수 있는 상태.

| 역할         | 작업                                                                                                                                  |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------- |
| **Backend**  | 공고 CRUD API (`/posts`), 카카오맵 Geocoding API 연동 (텍스트 → geometry 변환), `volunteer_schedules` 저장 API                        |
| **Frontend** | 공고 게시판 (목록/상세), 공고 등록 폼 (보호소), 챗봇 UI (말풍선 형태, 상태 머신 연동), 게시판 → 챗봇 진입 시 공고 요약 카드 자동 표시 |
| **AI**       | 챗봇 상태 머신 완성 및 Backend 연동, post_id 있을 때 자동 입력 처리, Geocoding 실패 시 재입력 처리                                    |

---

### 4주차: LLM 매칭 엔진

> **목표:** 자정 배치 매칭이 실제로 돌아가는 상태. 보호소가 매칭 결과를 확인할 수 있어야 함.

| 역할         | 작업                                                                                                                                           |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------- |
| **Backend**  | 자정 배치 스케줄러 구현, `relay_chains` / `relay_segments` 생성 로직, 배치 실패 처리 (D-3/D-2/D-1 에스컬레이션), 매칭 수락/거절 API            |
| **Frontend** | 매칭 제안 수락/거절 UI (봉사자), 보호소 대시보드 기본 구조 (공고 목록 + 상태 표시)                                                             |
| **AI**       | 1단계 SQL 필터 쿼리 작성, 2단계 Python 체인 알고리즘 (`can_chain`, `build_valid_chains`), 3단계 LLM 프롬프트 + JSON 파싱 안전장치 (재시도 2회) |

**주차 말 확인 항목**

- 매칭 엔진 E2E 테스트 (봉사자 3명 등록 → 자정 배치 → relay_segments 생성 확인)

---

### 5주차: 알림 시스템

> **목표:** Web Push, WebSocket, 이메일 알림 인프라 완성.

| 역할         | 작업                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Backend**  | Web Push 서버 구현 (VAPID 키 발급, 구독 저장), SendGrid/SES 이메일 연동, Redis Pub/Sub → WebSocket 구현, `GET /notifications/unread` API (재연결 폴백용) |
| **Frontend** | Web Push 권한 요청 UI (iOS 설치 안내 포함), WebSocket 연결 관리 (재연결 시 unread 조회), 인앱 알림 토스트 컴포넌트, 핑 체크 응답 UI                      |
| **AI**       | 핑 미응답 감지 로직 (출발 2시간 전 발송, 1시간 전 미응답 시 보호소 경고 트리거)                                                                          |

---

### 6주차: 릴레이 트래킹

> **목표:** 이동 중 체크포인트 기록, 인계 확인, 입양자 조회 페이지 동작.

| 역할         | 작업                                                                                                                                                              |
| ------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Backend**  | 체크포인트 API (`POST /relay/checkpoint`), 인계 코드 생성/검증 API (rate limit 포함), 인계 장소 변경 API, share_token 조회 API (공개 필드 화이트리스트)           |
| **Frontend** | 체크포인트 버튼 UI (출발·거점·도착), 인계 코드 입력 UI + 폴백(인계 완료 요청/승인), 카카오맵 연동 (경로 안내 URL 스킴), 입양자 조회 페이지 (위치 마커 + 타임라인) |
| **AI**       | 체크포인트 지연 감지 로직 (예상 도착 시간 대비 지연 여부 판단)                                                                                                    |

---

### 7주차: 예외 처리 + Admin

> **목표:** SOS, 지연, 노쇼, 관리자 기능 완성.

| 역할         | 작업                                                                                                                                                                           |
| ------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Backend**  | SOS API (긴급 재매칭 → 인근 봉사자 조회 → 관리자 이메일), 지연 신고 API, 노쇼 자동 30일 정지 처리, Admin API (보호소 승인, 사용자 정지/해제), share_token 만료 배치 + 비식별화 |
| **Frontend** | SOS 버튼 UI + 긴급 안내 메시지, 지연 알림 버튼 + 보호소 전화하기 버튼, 봉사 내역 페이지, Admin 페이지 (보호소 승인 목록, 사용자 관리)                                          |
| **AI**       | 긴급 재매칭 로직 (`backup_candidates` → 인근 봉사자 → 관리자 알림 순서 처리)                                                                                                   |

---

### 8주차: 통합 테스트 + 마무리

> **목표:** 전체 플로우 E2E 테스트, 버그 수정, 배포.

| 역할         | 작업                                                                            |
| ------------ | ------------------------------------------------------------------------------- |
| **공통**     | 전체 플로우 E2E 테스트 (공고 등록 → 챗봇 동선 입력 → 매칭 → 이동 → 인계 → 완료) |
| **Backend**  | 발견된 버그 수정, API 응답 속도 점검, 배포 환경 세팅                            |
| **Frontend** | 모바일 PWA 최종 점검 (iOS Safari, Android Chrome), 반응형 UI 점검               |
| **AI**       | LLM 매칭 결과 품질 점검, 프롬프트 튜닝, 이상 감지 오탐 점검                     |
...
2주차: 인증 (모든 기능의 전제)
    ↓
3주차: 공고 + 챗봇 (매칭 엔진의 입력 데이터)
    ↓
4주차: 매칭 엔진 (릴레이 트래킹의 전제)
    ↓
5주차: 알림 시스템 (예외 처리의 전제)
    ↓
6주차: 릴레이 트래킹
    ↓
7주차: 예외 처리 + Admin
...
- **매주 금요일:** 3팀 합동 싱크 (30분) — 전주 완료 확인 + 이번 주 블로커 공유
- **API 변경 시:** 변경 전날까지 공지 필수
- **4주차 말:** 중간 점검 — 매칭 엔진 E2E 동작 확인 후 후반부 일정 조정 여부 결정
