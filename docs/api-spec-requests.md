...
> **✅ 구현 완료** — `GET /shelter/dashboard` 응답에 `chain_expires_at` 포함. `status: "waiting"` 외에는 `null` 반환.
...
> **✅ 구현 완료** — `GET /shelter/dashboard` 응답에 `matching_reason` 포함. `status: "waiting"` 외에는 `null` 반환.
...
## 20. `GET /matching/segments/{segment_id}` 응답의 `kakao_openchat_url` 빈 문자열 버그

**요청:** 버그 수정

**현상:**
`matching_service.py`의 `get_segment` 함수 234번째 줄에 `"kakao_openchat_url": ""`이 하드코딩되어 있습니다.
FE에서는 이 필드가 있을 때만 오픈채팅 버튼을 렌더링하는데, 항상 빈 문자열이 반환되어 버튼이 표시되지 않습니다.

**수정 내용:**
```python
# 현재 (matching_service.py get_segment 함수 내)
"kakao_openchat_url": "",

# 수정 필요
"kakao_openchat_url": post.kakao_openchat_url or "" if post else "",
```

> `transport_posts` 테이블의 `kakao_openchat_url` 컬럼 값을 그대로 내려주면 됩니다.

**관련 파일:** `backend/app/services/matching_service.py:234`, `frontend/app/volunteer/matching/[segment_id]/page.tsx`

---

## 21. 세그먼트 거절 상태값 통일 요청 (`no_show` → `declined` 또는 병행 지원)

**요청:** 상태값 통일 또는 FE 처리 확인

**현상:**
`matching_service.py`의 `decline_segment`에서 거절 시 `segment.status = "no_show"`로 저장합니다.
FE에서는 `"declined"` 또는 `"no_show"` 모두 거절 화면을 표시하도록 수정했습니다 (`isDeclined = status === "declined" || status === "no_show"`).

**요청:**
- `"no_show"`가 의도된 공식 상태값이라면 api-spec.md에 명시해 주세요.
- 향후 FE/BE 간 혼동을 막기 위해 status enum을 api-spec.md에 명확히 기재해 주세요.

**현재 알려진 segment status 목록:**
| status | 의미 |
|--------|------|
| `pending` | 매칭 제안됨 (수락 대기) |
| `accepted` | 봉사자 수락 완료 |
| `in_progress` | 출발 체크포인트 기록 후 이동 중 |
| `completed` | 봉사 완료 (인계 또는 최종 도착) |
| `no_show` | 봉사자 거절 (BE 코드 기준) |

**관련 파일:** `backend/app/services/matching_service.py`, `frontend/app/volunteer/matching/[segment_id]/page.tsx`

---

## 22. `GET /matching/my-segments` — `completed`/`in_progress` 세그먼트 포함 요청

**요청:** `matching_repo.py` → `get_segments_for_volunteer` 필터 수정

**필요 이유:**
현재 `pending`/`accepted` 상태만 반환하여 완료된 봉사 세그먼트의 `segment_id`를 FE에서 알 수 없음.
봉사자가 최종 도착을 기록한 후 내 릴레이 > 완료 탭 카드를 눌러 릴레이 체인 요약 화면을 다시 볼 수 없음.
...
**관련 파일:** `backend/app/repositories/matching_repo.py`, `frontend/app/volunteer/services/page.tsx`

---

## 23. `mark_schedules_matched` — 사용된 스케줄만 matched 처리 요청

**요청:** `matching_repo.py` → `mark_schedules_matched` 로직 수정

**필요 이유:**
현재 봉사자가 세그먼트를 수락하면 같은 날짜의 **모든** 스케줄이 `matched`로 변경됨.
예: 마루(부산→대전) 수락 시 챗봇으로 따로 등록한 대전→서울 동선까지 `matched`로 처리되어 FE에서 상태 표시 오류 발생.

**필요한 변경:** 수락한 세그먼트에 연결된 스케줄(`volunteer_schedule_id`)만 `matched`로 변경.

**관련 파일:** `backend/app/repositories/matching_repo.py`, `backend/app/services/matching_service.py`
