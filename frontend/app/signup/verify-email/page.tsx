    ...
    <main className="flex min-h-screen flex-col items-center justify-center bg-white px-6 py-12">
      ...
      <div className="w-full max-w-sm animate-slide-up" style={{ animationDelay: "0.1s" }}>
        ...
        <div className="mb-6 text-center">
          ...
          {email && (
            <p className="mt-2 text-[14px] font-semibold text-[#EEA968] break-all">
              {email}
            </p>
          )}
        </div>
        ...
        <div className="mb-6 flex flex-col gap-2.5">
          <div className="flex items-start gap-3 rounded-2xl bg-[#FDF3EC] px-4 py-3.5">
        ...
        <Link
          href="/login"
          className="flex h-14 w-full items-center justify-center rounded-2xl bg-[#EEA968] text-[15px] font-bold text-white shadow-md shadow-[#EEA968]/20 transition-all duration-150 active:scale-[0.97]"
        >
          로그인 화면으로 이동
        </Link>
...
export default function VerifyEmailPage() {
  return (
    <Suspense fallback={<div className="flex min-h-screen items-center justify-center"><div className="h-6 w-6 animate-spin rounded-full border-2 border-[#EEA968] border-t-transparent" /></div>}>
      <VerifyEmailContent />
    </Suspense>
  );
}
