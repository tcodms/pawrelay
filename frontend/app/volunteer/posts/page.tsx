...
import { DUMMY_POSTS, URGENT_POST_IDS } from "@/lib/dummy-posts";

const URGENT_POSTS = DUMMY_POSTS.filter((p) => URGENT_POST_IDS.includes(p.id));
...
function PostsPage() {
  ...
  // URL 쿼리 파라미터에서 필터 상태 읽기 — 뒤로가기 시 필터 유지
  ...
  const allPosts = DUMMY_POSTS.filter((p) => p.status === "recruiting");
      ...
      <section className="mx-auto max-w-2xl mt-5 px-4 pb-8">
        <div className="flex items-center justify-between mb-3">
          <p className="text-[17px] font-bold text-gray-800">
            전체 공고 <span className="text-[#EEA968]">{recruitingPosts.length}</span>
          </p>
