...
import { sendChatMessage, CHATBOT_POST_CONTEXT_KEY } from "@/lib/api/chatbot";
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  const res = await sendChatMessage(null, post.id, null);
                  sessionStorage.setItem(`chatbot_init_${res.session_id}`, JSON.stringify(res));
                  router.push(`/volunteer/chat/${res.session_id}`);
                } catch {
                  alert("신청을 시작할 수 없습니다. 다시 시도해 주세요.");
                } finally {
                  setApplying(false);
                }
              }}
              className="flex w-full items-center justify-center h-14 rounded-full bg-[#EEA968] text-[15px] font-bold text-white shadow-md shadow-[#EEA968]/20 transition-all active:scale-[0.97] hover:bg-[#D99A55] disabled:opacity-60"
            >
              {applying ? (
